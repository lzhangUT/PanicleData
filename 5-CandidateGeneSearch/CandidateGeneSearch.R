...
setwd('C://Users/Li Zhang/Desktop/PanicleData/5-CandidateGeneSearch/')
### Read QTL info
QTL_all = read.csv('QTLlistForAllTraits_flankmarker.csv')
...
RiceGene = read.csv('RiceGenes.csv')
##Read gff3 file and annotation file
gff3 = read.gff('Pvirgatum_516_v5.1.gene.gff3.gz')
...
#######get the candidate genes for each QTL intervel for the panicle traits
Counts = NULL
...
for (i in 1:nrow(QTL)){ 
  ...
  counts = data.frame(QTL=QTL$QTL[i], pvGenes=length(gene_list$locusName), ArabGenes=length(unique(gene_list$Best.hit.arabi.name))-1,
                      RiceGenes=length(unique(gene_list$Best.hit.rice.name))-1, RiceGenesCloned=length(unique(gene_list$OsGene))-1)
...
save.image('CandidateGeneSearch_PanicleData.RData')
